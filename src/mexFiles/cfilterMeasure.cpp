...
void matrixPrint(const Eigen::MatrixXd mat);
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	//INPUTS
 	...
 	Eigen::MatrixXd xkBar,PkBar,Rk,RBI,L_s2p,L_cg2p, rI_measurement, rCu_measurement;
 	xkBar = datastore[0];
 	PkBar = datastore[1];
 	Rk = datastore[2];
 	RBI = datastore[3];
 	L_s2p = datastore[4];
 	L_cg2p = datastore[5];
 	rI_measurement = datastore[6];
 	rCu_measurement = datastore[7];

 	//PROCESSING
 	Eigen::MatrixXd matrixToReturn;
 	Eigen::Matrix<double,15,1> xkp1;
 	Eigen::Matrix<double,15,15> Pkp1;
 	spkfMeasure6(xkBar, PkBar, Rk, rI_measurement, rCu_measurement, RBI, L_cg2p, L_s2p, Pkp1, xkp1);
	...
	const Eigen::Matrix<double,6,1> vk)
{
	...
	Eigen::Vector3d rCB = R2.transpose()*unit3(ls2p) + vk.bottomRows(3); //NOTE: unit3(meas+noise) vs unit3(meas)+noise
	...
	const Eigen::Matrix3d RR, const Eigen::Vector3d lAB, Eigen::Matrix<double,15,15> &Pkp1, Eigen::Matrix<double,15,1> &xkp1)
{
	...
	cholP = (Paug.llt().matrixL()); //NOTE: chol(P) in matlab returns U, chol(P) in Eigen returns L
