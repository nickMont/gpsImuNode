...
//  internalPosePub_ = nh.advertise<geometry_msgs::PoseStamped>(posePubTopic,10);
    ...
    imuFilter.setLevers(l_cg2p,l_imu,l_s2p);
    imuFilter.setCovariances(dtIMU, Qk12, Pimu, Rk);
    imuFilter.setImuParams(tauA,tauG);
    imuFilter.setBiasSaturationLimits(maxBa, maxBg);
...
void estimationNode::imuDataCallback(const gbx_ros_bridge_msgs::Imu::ConstPtr &msg)
{
    ...
    if(isCalibrated)
    {
        ...
        { 
            ...
            imuFilter.runUKFpropagateOnly(tLastProcessed,thisImuMeas);
