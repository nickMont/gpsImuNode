...
class gpsImu
{
	public:
        ...
        void runUKFpropagateOnly(const double tPrev, const imuMeas &imu);

        //Setters. Everything must be initialized piecemeal. This is preferred as some things are not known
        ...
        void setCovariances(const double dt, const Eigen::Matrix<double,12,12> &Qin, const Eigen::Matrix<double,15,15> &Pin,
            const Eigen::Matrix<double,6,6> &Rin) {Qk12_=Qin;  Pimu_=Pin;  Rk_=Rin; 
                Qk12dividedByDt_=Eigen::Matrix<double,12,12>::Zero();  Qk12dividedByDt_=Qin/dt;}
